...
async function register(name, email, password, apiKey) {
  const payloadReg = {
    ...
    email: email,
    password: password,
    referral_code: "1PSC3ZSUV5Y3",
  };
  const response = await coday("https://api.meshchain.ai/meshmain/auth/email-signup", "POST", headers, payloadReg);
...
async function login(email, password, apiKey) {
  ...
  const response = await coday("https://api.meshchain.ai/meshmain/auth/email-signin", "POST", headers, payloadLogin);
...
async function verify(email, otp, apiKey) {
  ...
  const response = await coday("https://api.meshchain.ai/meshmain/auth/verify-email", "POST", headers, payloadVerify);
...
async function claimBnb() {
  ...
  const response = await coday("https://api.meshchain.ai/meshmain/mission/claim", "POST", headers, payloadClaim);
...
async function main() {
  try {
    ...
    try {
      ...
      await fs.appendFile("token.txt", `${loginData.access_token}|${loginData.refresh_token}\n`, "utf-8");
